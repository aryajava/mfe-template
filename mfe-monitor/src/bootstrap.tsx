...
const getRolePermissionMatrix = (role: string): Record<string, MenuPermissionItem> => {
  ...
  return {
    'permintaan-diskon': {
      menuCode: 'permintaan-diskon',
      groupCode: 'monitoring',
      groupName: 'Monitoring',
      urlPrefix: '/permintaan-diskon',
      fullPath: '/monitoring/permintaan-diskon',
      canRead: true,
      canCreate: true,
      canUpdate: true,
      canDelete: false,
    ...
    'persetujuan-diskon': {
      menuCode: 'persetujuan-diskon',
      groupCode: 'monitoring',
      groupName: 'Monitoring',
      urlPrefix: '/persetujuan-diskon',
      fullPath: '/monitoring/persetujuan-diskon',
...
const getStandaloneAuthContext = (): AuthContextType => {
  ...
  try {
    ...
    if (raw) {
      user = JSON.parse(raw);
      role = (user.roles?.[0] || 'OWNER').toUpperCase();
    }
  } catch {}
  ...
  return {
    ...
    canAccessMenu: (menuCodeOrPath: string) => {
      ...
      if (lower === 'monitoring' || lower === 'monitor' || lower === '') return true;
      const item =
